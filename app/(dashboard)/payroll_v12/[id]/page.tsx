...
export default function PayrollDetailPageV12() {
  ...
  const loadData = async () => {
    const orgId = typeof document !== 'undefined'
      ...
      : ''
    if (!orgId || !id) return
    const [periodRes, rowsRes, membersRes] = await Promise.all([
      fetch(`/api/payroll/periods/list?org_id=${orgId}&limit=200`, { cache: 'no-store' }),
      fetch(`/api/payroll/members/list?payroll_period_id=${id}`, { cache: 'no-store' }),
      fetch(`/api/user/list?orgId=${orgId}`, { cache: 'no-store' })
    ])
    const periodData = await periodRes.json()
    const rowsData = await rowsRes.json()
    const membersData = await membersRes.json()
    setPeriod(periodData.items?.find((p: any) => p.id === id) || null)
  ...
  const rowsViz = rows.map(r => [
    memberMap.get(r.member_id) || r.member_id,
    String(Math.round(r.worked_minutes/60)),
    String(Math.round(r.extra_minutes/60)),
    String(Math.round(r.short_minutes/60)),
    `$${Math.round(r.base_salary).toLocaleString()}`,
    `$${Math.round(r.overtime_amount).toLocaleString()}`,
    `$${Math.round(r.short_deduction).toLocaleString()}`,
    `$${Math.round(r.fines_total).toLocaleString()}`,
    `$${Math.round(r.adjustments_total).toLocaleString()}`,
    `$${Math.round(r.net_salary).toLocaleString()}`,
    <div className="row" style={{ gap:8 }}>
      <GlassButton
        variant="secondary"
        onClick={() => {
          setTargetRow(r)
          setAdj({ type: 'bonus', amount: 0, reason: '' })
          setAdjOpen(true)
        }}
