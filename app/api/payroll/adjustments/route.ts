...
import { addAdjustment, listAdjustments } from '@lib/db'
...
export async function POST(req: NextRequest) {
  ...
  if (!memberId || !orgId || !date || !reason || (amount === undefined || amount === null)) return NextResponse.json({ error: 'MISSING_FIELDS' }, { status: 400 })
  const r = await addAdjustment({ memberId, orgId, date, reason, amount, currency, createdBy: actor })
