...
export async function GET(req: NextRequest) {
  ...
  if (sb) {
    ...
    totalWorkedMinutes = daily.reduce((s: number, r: any) => s + Number(r.worked_minutes || 0), 0)
    ...
    presentDays = daily.filter((r: any) => Number(r.worked_minutes || 0) > 0).length
  ...
  if (sb) {
    ...
    const { data: evRows } = tsIds.length ? await sb.from('activity_events').select('app_name, category, is_active').in('tracking_session_id', tsIds) : { data: [] }
