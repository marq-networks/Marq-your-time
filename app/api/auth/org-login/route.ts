...
export async function POST(req: NextRequest) {
  ...
  if (!org && orgName) {
    if (isSupabaseConfigured()) {
      ...
      if (data) org = { id: String(data.id), orgName: String(data.org_name), orgLogo: data.org_logo ?? undefined, orgPasswordHash: data.org_password_hash ?? undefined }
    } else {
  ...
  if (org.orgPasswordHash && (!hash || org.orgPasswordHash !== hash)) {
    return NextResponse.json({ error: 'ORG_PASSWORD_INVALID' }, { status: 401 })
  }
