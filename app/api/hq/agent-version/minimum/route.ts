...
export async function GET(req: NextRequest) {
  ...
  if (!['super_admin', 'owner', 'admin'].includes(role)) return NextResponse.json({ error: 'FORBIDDEN' }, { status: 403 })
  ...
  const v = data?.value_json || { minimum_version: '1.0.0', download_url: '', download_url_windows: '', download_url_mac: '', block_below: false }
  return NextResponse.json({ 
    minimum_version: v.minimum_version || '1.0.0', 
    download_url: v.download_url || '', 
    download_url_windows: v.download_url_windows || '',
    download_url_mac: v.download_url_mac || '',
    block_below: !!v.block_below 
  })
}
...
export async function POST(req: NextRequest) {
  ...
  if (!sb) {
    ...
    const download_url_windows = body.download_url_windows || body.downloadUrlWindows || ''
    const download_url_mac = body.download_url_mac || body.downloadUrlMac || ''
    ...
    return NextResponse.json({ minimum_version, download_url, download_url_windows, download_url_mac, block_below })
  }
  const body = await req.json().catch(()=>({}))
  const minimum_version = body.minimum_version || body.minimumVersion
  const download_url = body.download_url || body.downloadUrl || ''
  ...
  const value_json = { minimum_version, download_url, download_url_windows, download_url_mac, block_below }
  if (existing) {
