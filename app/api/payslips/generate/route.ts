...
import { generatePayslipsForPeriod } from '@lib/payslips'
...
export async function POST(req: NextRequest) {
  ...
  const res = await generatePayslipsForPeriod(orgId, period)
  if (!('ok' in res) || !res.ok) return NextResponse.json(res, { status: 400 })
