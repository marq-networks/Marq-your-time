...
export async function GET(req: NextRequest) {
  ...
  // Fetch users once to use for mapping and filtering
  ...
  if (!sb) {
    ...
    // Map member details for memory items
    items = items.map((it: any) => {
        const u = userMap.get(it.member_id || it.memberId)
        return {
            ...it,
            member: u ? {
    ...
    if (department_id || manager_id || member_role_id) {
      const allow = new Set(allUsers.filter(u => (!department_id || u.departmentId === department_id) && (!manager_id || u.managerId === manager_id) && (!member_role_id || u.memberRoleId === member_role_id)).map(u => u.id))
